...
function Resolve-AppPath {
  ...
  if ([string]::IsNullOrWhiteSpace($PathValue)) {
    return Join-Path (Join-Path $AppRoot "data") "azure-cert-gui.sqlite"
  }
...
function Get-RuntimeEnvPath {
  return Get-RuntimeEnvPowerShellPath
}

function Get-RuntimeEnvPowerShellPath {
  return Join-Path (Join-Path $AppRoot ".runtime") "azure-cert-gui.env.ps1"
}

function Get-RuntimeEnvFilePath {
  return Join-Path (Join-Path $AppRoot ".runtime") "azure-cert-gui.env"
}
...
function Read-RuntimeEnvValues {
  ...
  foreach ($line in Get-Content -Path $Path) {
    ...
    } elseif ($line -match "^\s*`$env:([A-Za-z0-9_]+)\s*=\s*`"(.*)`"\s*$") {
      $values[$Matches[1]] = ($Matches[2] -replace '``"', '"')
    }
  }
  return $values
...
function Write-RuntimeEnvFile {
  ...
  $path = Get-RuntimeEnvPowerShellPath
  $envPath = Get-RuntimeEnvFilePath
  $powerShellLines = @(
    '$ErrorActionPreference = "Stop"',
  ...
  foreach ($key in ($Values.Keys | Sort-Object)) {
    ...
    $powerShellLines += "`$env:$key = $(Quote-PowerShellString $value)"
    $envLines += "$key=$(Quote-EnvFileString $value)"
  }
  Set-Content -Path $path -Value ($powerShellLines -join [Environment]::NewLine) -Encoding UTF8
...
function Write-BaseRuntimeEnv {
  param(
    [string]$ResolvedDatabasePath,
    [int]$ResolvedPort,
    [string]$ResolvedHost
  )
  $values = Read-RuntimeEnvValues -Path (Get-RuntimeEnvPowerShellPath)
  $values["AZURE_CERT_GUI_DB_PATH"] = $ResolvedDatabasePath
  $values["UI_HOST"] = $ResolvedHost
  $values["PORT"] = [string]$ResolvedPort
  $values["NODE_ENV"] = "production"
  if (-not $values.ContainsKey("AZURE_CERT_GUI__AUTH__MODE")) {
    $values["AZURE_CERT_GUI__AUTH__MODE"] = "local"
  }
  if (($values["AZURE_CERT_GUI__AUTH__MODE"] -eq "local") -and -not $values.ContainsKey("AZURE_CERT_GUI__AUTH__ALLOWLOCALINPRODUCTION")) {
    $values["AZURE_CERT_GUI__AUTH__ALLOWLOCALINPRODUCTION"] = "true"
  }
  return Write-RuntimeEnvFile -Values $values
}
...
function Write-SystemdUserUnits {
  if (-not (Test-IsLinux)) {
    throw "-WriteSystemdUserUnits is supported only on Linux."
  }
  $node = Get-ExecutablePath @("node")
  if (-not $node) {
    throw "node was not found on PATH after Node.js setup."
  }
  Push-Location $AppRoot
  try {
    Invoke-Checked -Command $node -Arguments @("scripts/write-systemd-user-units.mjs")
  } finally {
    Pop-Location
  }
}
...
$runtimeEnvPath = Write-BaseRuntimeEnv -ResolvedDatabasePath $resolvedDatabasePath -ResolvedPort $Port -ResolvedHost $UiHost
Write-Host "Runtime env: $runtimeEnvPath"
Write-Host "Runtime env: $(Get-RuntimeEnvFilePath)"
...
if ($WriteSystemdUserUnits) {
  Write-Step "Writing systemd user units"
  Write-SystemdUserUnits
}
